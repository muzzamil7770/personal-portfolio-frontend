...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        <h1>
          ...
          Hire Requests <span class="badge">{{ hires.length }}</span>
        </h1>
      </div>
      ...
      <!-- Skeleton -->
      <app-skeleton *ngIf="loading" variant="table" [rows]="5" [cols]="7"></app-skeleton>
        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let h of hires">
              ...
              <td class="td-budget">{{ h.budget || '—' }}</td>
              <td class="td-services">{{ h.services || '—' }}</td>
              <td>
                <div class="status-wrap">
                  <span class="status" [class]="h.status">{{ h.status }}</span>
                  <div class="status-actions">
                    <button class="sa-btn sa-ok" title="Mark completed" (click)="quickStatus(h, 'completed')">
                      <i class="fas fa-check"></i>
                    </button>
                    <button class="sa-btn sa-no" title="Mark rejected" (click)="quickStatus(h, 'rejected')">
                      <i class="fas fa-times"></i>
                    </button>
                  </div>
                </div>
              </td>
              <td class="td-date">{{ h.createdAt | date:'dd MMM yy' }}</td>
              <td class="actions">
                <button class="btn-view" (click)="openView(h.id)">
                  <i class="fas fa-eye"></i> View
                </button>
                <button class="btn-del" (click)="delete(h.id)">
            ...
            <tr *ngIf="hires.length === 0">
              <td colspan="7" class="empty">
                <i class="fas fa-inbox"></i> No hire requests found.
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="md-dialog" *ngIf="dialogOpen" role="dialog" aria-modal="true">
      <div class="md-dialog-header">
        <div class="md-dialog-title-wrap">
          ...
          <div>
            <div class="md-dialog-title">{{ editMode ? 'Edit Hire Request' : 'Hire Request Details' }}</div>
      ...
      <app-skeleton *ngIf="modalLoading" variant="dialog" [rows]="5"></app-skeleton>
      ...
      <div class="md-view-body" *ngIf="!modalLoading && form && !editMode">
        ...
        <div class="vf-row">
          <div class="vf-item">
            <span class="vf-label"><i class="fas fa-dollar-sign"></i> Budget</span>
            <span class="vf-val vf-budget">{{ form.budget || '—' }}</span>
          </div>
          <div class="vf-item">
        ...
        <div class="vf-item" *ngIf="form.services">
          <span class="vf-label"><i class="fas fa-tools"></i> Services</span>
          <span class="vf-val">{{ form.services }}</span>
        </div>
      ...
      <form class="md-dialog-body" *ngIf="!modalLoading && form && editMode" (ngSubmit)="save()">
        ...
        <div class="md-row">
          <div class="md-field">
            <label class="md-label">Budget</label>
            <input class="md-input" [(ngModel)]="form.budget" name="budget" placeholder="e.g. $500–$1000" />
          </div>
          <div class="md-field">
        ...
        <div class="md-field">
          <label class="md-label">Services</label>
          <input class="md-input" [(ngModel)]="form.services" name="services" placeholder="e.g. react-app, web-development" />
        </div>
        <div class="md-field">
  ...
  styles: [`
    ...
    .td-budget { font-family: monospace; font-size: 0.82rem; color: #22c55e; font-weight: 600; }
    .td-services { max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 0.8rem; }
    ...
    .vf-budget { color: #22c55e; font-family: monospace; font-weight: 600; }
    .vf-msg { gap: 6px; }
    .vf-message {
    ...
    .md-row { display: grid; grid-template-columns: 1fr 1fr; gap: 0.9rem; }
    @media(max-width:480px) { .md-row { grid-template-columns: 1fr; } }
...
export class HiresComponent implements OnInit {
  ...
  load() {
    ...
    this.api.getHires().subscribe({
      next: res => { this.hires = res.data; this.loading = false; },
      error: () => { this.errorMsg = 'Failed to load hire requests.'; this.loading = false; }
    });
  }
  ...
  quickStatus(h: Hire, status: string) {
    this.api.updateHire(h.id, { status } as any).subscribe({
      next: () => { h.status = status; this.toast.success('Updated', `Marked as ${status}.`); },
  ...
  async save() {
    ...
    this.api.updateHire(this.editId, payload).subscribe({
      next: () => { this.closeDialog(); this.load(); this.toast.success('Saved!', 'Hire request updated.'); },
  ...
  async delete(id: string) {
    const result = await Swal.fire({
      title: 'Delete Hire Request?', text: 'This cannot be undone!', icon: 'warning',
    ...
    this.api.deleteHire(id).subscribe({
      next: () => { this.load(); this.toast.success('Deleted!', 'Hire request removed.'); },
